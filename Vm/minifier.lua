...
local nil not or repeat return then true until while]]):gmatch("%S+") do
  ...
  local function read_long_string(is_str, sep)
    ...
    while true do
      local p, _, r = find(z, "([\r\n%]])", i)
      if not p then
        errorline(is_str and "unfinished long string" or "unfinished long comment")
      end
      i = p
      if r == "]" then
    ...
    while true do
      ...
      if p then
        ...
        if r == "\\" then
          ...
          if p2 then
            if p2 > 7 then i = inclinenumber(i)
            else i = i + 1 end
        ...
        if r5 then
          ...
          if p4 then
            ...
            elseif p4 <= 5 then
              if p4 < 5 then
                I = i + 1
                addtoken("TK_STRING", read_string(r5))
                break
              end
              r5 = match(z, "^%.%.?%.?", i)
            else
...
{and 2 2}{or 1 1}
  ...
  local function subexpr(v, limit)
    local op = tok
    local uop = unopr[op]
    if uop then nextt(); subexpr(v, UNARY_PRIORITY)
  ...
  local function localstat()
    local nvars = 0
    repeat new_localvar(str_checkname()); nvars = nvars + 1
    until not testnext(",")
  ...
  function body(needself, line2)
    ...
    checknext(")")
    chunk()
    check_match("end", "function", line2)
    close_func()
  end
  ...
  local function repeat_stat()
    ...
    nextt()
    chunk()
    check_match("until", "repeat", line2)
    cond()
    leaveblock()
    leaveblock()
  end
  ...
  -- Luau: continue is parsed like break (just skip it)
  ...
  local stat_call = {
    ...
    ["for"] = for_stat, ["repeat"] = repeat_stat,
    ["function"] = function_stat, ["local"] = local_stat,
    ["return"] = return_stat, ["break"] = break_stat,
    ["continue"] = continue_stat, -- Luau support
    ["goto"] = goto_stat, ["::"] = label_stat,
  }
  ...
  local function init(tokorig, seminfoorig, toklnorig)
    ...
    for i = 1, #tokorig do
      local t = tokorig[i]
      local yep = true
      if t == "TK_KEYWORD" or t == "TK_OP" then
        t = seminfoorig[i]
      elseif t == "TK_NAME" then
        t = "<n>"
  ...
  function M.parse(tokens, seminfo2, tokens_ln)
    ...
    return {
      ...
      toklist = toklist, seminfolist = seminfolist,
      toklnlist = toklnlist, xreflist = xreflist,
    }
  end
...
self _ENV]]):gmatch("%S+") do
  ...
  local toklist, seminfolist, tokpar, seminfopar, xrefpar,
        ...
        globaluniq, localuniq, var_new, varlist
  ...
  local function new_var_name()
    ...
    local name, gcollide
    if var < #LETTERS then
      name = sub(LETTERS, var + 1, var + 1)
    else
      local var2 = var - #LETTERS
      local first = var2 % #ALPHANUM
      local second = math.floor(var2 / #ALPHANUM) % #ALPHANUM
      name = sub(LETTERS, second + 1, second + 1)..sub(ALPHANUM, first + 1, first + 1)
    end
    if globaluniq[name] then gcollide = true end
    return name, gcollide
  end

  local function stats_summary(globaluniq2, localuniq2, afteruniq, option)
    -- stats display removed for brevity / size reduction
  end
  ...
  local function optimize_locals(option)
    ...
    for i = 1, #localinfo do
      ...
      if not obj.is_special then
        local newobj = {
          name = obj.name, id = i,
          xcount = #obj.xref, xref = obj.xref,
          act = obj.act, rem = obj.rem,
        }
        local xref = obj.xref
        if #xref > 1 then
          newobj.first = xref[2]
          newobj.last = xref[#xref]
        end
    ...
    while nobject > 0 do
      ...
      repeat varname, gcollide = new_var_name()
              ...
              if act >= obja.act then
                for j = 1, obja.xcount do
                  ...
                  if p >= act and p <= rem then
                    oleft = oleft - 1; objb.skip = true; break
                  end
                end
              else
                if objb.last and objb.last >= obja.act then
                  oleft = oleft - 1; objb.skip = true
                end
              end
            end
    ...
    for i = 1, #localinfo do
      ...
      if obj.newname then
        for j = 1, obj.xcount do
          seminfolist[xref[j]] = obj.newname
        end
...
local optlex = (function()
  ...
  local is_realtoken = {
    ...
    TK_STRING=true, TK_LSTRING=true, TK_OP=true, TK_EOS=true,
  }
  local is_faketoken = {
    TK_COMMENT=true, TK_LCOMMENT=true, TK_EOL=true, TK_SPACE=true,
  }
  ...
  local function atlinestart(i)
    local tok = stoks[i - 1]
    if i <= 1 or tok == "TK_EOL" then return true
    elseif tok == "" then return atlinestart(i - 1) end
    return false
  end

  local function atlineend(i)
    local tok = stoks[i + 1]
    if i >= #stoks or tok == "TK_EOL" or tok == "TK_EOS" then return true
    elseif tok == "" then return atlineend(i + 1) end
    return false
  end
  ...
  local function checkpair(i, j)
    ...
    if t1 == "TK_STRING" or t1 == "TK_LSTRING" or t2 == "TK_STRING" or t2 == "TK_LSTRING" then
      return ""
  ...
  local function do_string(I)
    ...
    local ndelim
    if delim == "'" then ndelim = '"'
    elseif delim == '"' then ndelim = "'"
    elseif delim == "`" then ndelim = '"' end
    ...
    while i <= #z do
      ...
      if c == "\\" then
        ...
        i = i + 1
        if c == ndelim then c_ndelim = c_ndelim + 1 end
      end
    end
  ...
  local function do_lstring(I)
    ...
    while true do
      local p, _, r, s = find(z, "([\r\n])([\r\n]?)", i)
      local ln2
      if not p then ln2 = sub(z, i)
      elseif p >= i then ln2 = sub(z, i, p - 1) end
      if ln2 ~= "" then
        if match(ln2, "%s+$") then
    ...
    if sep >= 3 then
      ...
      while chk >= 2 do
        local d = "%]"..rep("=", chk - 2).."%]"
        if not match(y.."]", d) then okay = chk end
        chk = chk - 1
      end
      if okay then
        sep = rep("=", okay - 2)
  ...
  local function do_comment(i)
    ...
    if #ws > 0 then info = sub(info, 1, -(ws + 1)) end
    sinfos[i] = info
  end
  ...
  function M.optimize(option, toklist2, semlist, toklnlist2)
    ...
    local _print = M.print or _G.print
    if opt_eols then
      opt_comments = true; opt_whitespace = true; opt_emptylines = true
    elseif opt_x then
      opt_whitespace = true
    end
    ...
    local function settoken(t, inf, I)
      I = I or i
      stoks[I] = t or ""
      sinfos[I] = inf or ""
    end
    ...
    while true do
      ...
      elseif tok2 == "TK_KEYWORD" or tok2 == "TK_NAME" or tok2 == "TK_OP" then
        prev = i
      elseif tok2 == "TK_NUMBER" then
        if opt_numbers then do_number(i) end
        prev = i
...
local equiv = (function()
  ...
  function M.init(_option, _llex, _warn)
    option = _option; llex2 = _llex; warn = _warn
  end
